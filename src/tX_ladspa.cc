    ...
    for (i = 0; (descriptor = desc_func(i)) != NULL; i++) {
        if (LADSPA_IS_INPLACE_BROKEN(descriptor->Properties)) {
            ...
            for (port = 0; port < descriptor->PortCount; port++) {
                if (LADSPA_IS_PORT_AUDIO(descriptor->PortDescriptors[port])) {
                    if (LADSPA_IS_PORT_INPUT(descriptor->PortDescriptors[port]))
                        in_audio++;
                    else if (LADSPA_IS_PORT_OUTPUT(descriptor->PortDescriptors[port]))
                        out_audio++;
                } else if (LADSPA_IS_PORT_CONTROL(descriptor->PortDescriptors[port]) && LADSPA_IS_PORT_INPUT(descriptor->PortDescriptors[port]))
                    in_ctrl++;
            }
            ...
            if ((in_audio == 2) && (out_audio == 2)) {
                ...
                tX_plugin_warning("Plugin \"%s\" [%li] disabled: Neither mono nor stereo.", descriptor->Label, descriptor->UniqueID);
            }
        }
    }
}
...
void LADSPA_Plugin ::scandir(char* dirname) {
    ...
    if (!dir) {
        tX_error("couldn't access directory \"%s\".", dirname);
        return;
    };
...
LADSPA_Stereo_Plugin* LADSPA_Stereo_Plugin ::getPluginByIndex(int i) {
    ...
    if (plugin == stereo_plugin_list.end())
        return NULL;
