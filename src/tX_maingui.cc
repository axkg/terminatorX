...
#define connect_entry(wid, func, ptr) \
    ;                                 \
    g_signal_connect(G_OBJECT(wid), "activate", (GCallback)func, (void*)ptr);
#define connect_adj(wid, func, ptr) \
    ;                               \
    g_signal_connect(G_OBJECT(wid), "value_changed", (GCallback)func, (void*)ptr);
#define connect_button(wid, func, ptr) \
    ;                                  \
    g_signal_connect(G_OBJECT(wid), "clicked", (GCallback)func, (void*)ptr);
...
void do_save_tables(char* buffer) {
    ...
    if (out || zout) {
        if (vtt_class::save_all(out, zout))
            tx_note("Error while saving set.", true);
        if (out)
            fclose(out);
        else if (zout)
            gzclose(zout);
    ...
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(w))) {
        ...
        if (res != NO_ERROR) {
            ...
            case ERROR_TAPE:
                ...
                break;
            default:
                tx_note("Error starting engine: Unknown error.", true);
            }
...
void create_maingui(int x, int y) {
    ...
    dummy = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2);
...
void remove_from_panel_bar(GtkWidget* button) {
    ...
    if (buttons_on_panel_bar == 0)
        gtk_widget_hide(panel_bar);
}
...
void tX_cursor::set_cursor(cursor_shape shape) {
    ...
    case WAIT_CURSOR:
        if (!cursors[shape])
            cursors[shape] = gdk_cursor_new_for_display(display, GDK_WATCH);
        break;
