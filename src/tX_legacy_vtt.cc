...
#define atload(data)                                      \
    ;                                                     \
    if (fread((void*)&data, sizeof(data), 1, input) != 1) \
        res += 1;
...
int vtt_class ::load_all_14(FILE* input, char* fname) {
    ...
    for (i = 0; i < max; i++) {
        ...
        gtk_box_pack_start(GTK_BOX(control_parent), newvtt->gui.control_box, TRUE, TRUE, 0);
        gtk_box_pack_start(GTK_BOX(audio_parent), newvtt->gui.audio_box, TRUE, TRUE, 0);
        if (newvtt->audio_hidden)
            newvtt->hide_audio(newvtt->audio_hidden);
        if (newvtt->control_hidden)
            newvtt->hide_control(newvtt->control_hidden);
    }
...
void tX_sequencer ::load(FILE* in) {
    ...
    if (new_event)
        max_timestamp = new_event->get_timestamp();
}
