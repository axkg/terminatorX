...
vtt_fx_ladspa ::vtt_fx_ladspa(LADSPA_Plugin* p, void* v) {
    ...
    for (port = 0; port < plugin->getPortCount(); port++) {
        if (LADSPA_IS_PORT_AUDIO(cpd)) {
            ...
            if (LADSPA_IS_HINT_BOUNDED_BELOW(cph.HintDescriptor))
                min = cph.LowerBound;
            if (LADSPA_IS_HINT_BOUNDED_ABOVE(cph.HintDescriptor))
                max = cph.UpperBound;
...
void vtt_fx_ladspa ::activate() {
    ...
    if (plugin->getDescriptor()->activate)
        plugin->getDescriptor()->activate(instance);
}

void vtt_fx_ladspa ::deactivate() {
    if (plugin->getDescriptor()->deactivate)
        plugin->getDescriptor()->deactivate(instance);
...
tX_drywet_type vtt_fx_ladspa::has_drywet_feature() {
    if (sp_wet)
        return DRYWET_ACTIVE;
    else
        return DRYWET_AVAILABLE;
}

/****** STEREO plugins **********/

vtt_fx_stereo_ladspa::vtt_fx_stereo_ladspa(LADSPA_Stereo_Plugin* p, void* v)
    : vtt_fx_ladspa(p, v) {
    ...
    for (int port = 0; port < plugin->getPortCount(); port++) {
        if (LADSPA_IS_PORT_AUDIO(cpd)) {
            if (LADSPA_IS_PORT_INPUT(cpd)) {
                if (input_port < 0) {
                    input_port = port;
                } else if (input2_port < 0) {
                    input2_port = port;
                } else {
                    tX_error("Extra input port for plugin %s?", plugin->getName());
                }
            } else if (LADSPA_IS_PORT_OUTPUT(cpd)) {
                if (output_port < 0) {
                    output_port = port;
                } else if (output2_port < 0) {
                    output2_port = port;
                } else {
                    tX_error("Extra output port for plugin %s?", plugin->getName());
                }
            }
        }
    }
