...
#define TX_DEFAULT_SIZE_X 200
#define TX_DEFAULT_SIZE_Y 100
...
gtk_tx_prepare (GtkWidget *widget)
{
	...
	tx->spp=tx->samples/widget->allocation.width;
	...
	tx->disp_data =(int16_t*) malloc(widget->allocation.width * sizeof(int16_t));

	if (tx->disp_data)
	for (x=0, ptr=tx->disp_data; x< widget->allocation.width; ptr++,x++)
	{
		...
		tx->disp_data[x]=(int) (temp * (f_prec) (tx->yc));
	}
...
void gtk_tx_cleanup_pos_display(GtkTx *tx)
{
	...
	gdk_flush();	// moved to update() due to some gcc/egcs strangeness
}
