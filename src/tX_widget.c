...
#define draw_line(x1, y1, x2, y2, rgba)      \
    {                                        \
...
#define draw_rectangle(rect, rgba)                                    \
    {                                                                 \
        gdk_cairo_set_source_rgba(cr, rgba);                          \
        cairo_rectangle(cr, rect.x, rect.y, rect.width, rect.height); \
        cairo_fill(cr);                                               \
    }
#define draw_rectangle2(x, y, width, height, rgba) \
    {                                              \
        gdk_cairo_set_source_rgba(cr, rgba);       \
...
static void gtk_tx_update_render_buffer(GtkTx* tx) {
    ...
    if ((tx->render_buffer_display_width != tx->display_width) || (tx->render_buffer_fg != tx->current_fg)) {
