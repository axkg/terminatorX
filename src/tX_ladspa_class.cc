...
void LADSPA_Class::scandir(char* dirname) {
    ...
    if (!dirlen) {
        tX_error("LADSPA_Class::scandir() Empty directory name");
        return;
    };
    ...
    if (!dir) {
        tX_error("LADSPA_Class::scandir() couldn't access directory \"%s\"", dirname);
        return;
    };
...
LADSPA_Class ::LADSPA_Class(const char* uri)
    : label(NULL)
    , accept_all(false) {
#ifdef USE_LRDF
    lrdf_uris* ulist;
...
bool LADSPA_Class ::add_plugin_instance(LADSPA_Plugin* plugin, LADSPA_Plugin_Type type) {
    ...
    for (cls = subclasses.begin(); cls != subclasses.end(); cls++) {
        ...
        if (lrdf_class->add_plugin_instance(plugin, type))
            return true;
    }
...
void LADSPA_Class::list(char* buffer) {
    ...
    for (c = subclasses.begin(); c != subclasses.end(); c++)
        (*c)->list(buffer);
