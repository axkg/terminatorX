...
void tx_mouse::key_press(GtkWidget* widget, GdkEventKey* eventKey) {
    if (vtt) {
        ...
        case GDK_KEY_space:
            ...
            break;
        case GDK_KEY_Escape:
            if (press_key(KEY_Escape)) {
                ungrab();
            }
            break;

        case GDK_KEY_Return:
            if (press_key(KEY_Return)) {
                vtt->sp_trigger.receive_input_value(1);
            }
            break;
        case GDK_KEY_BackSpace:
            if (press_key(KEY_BackSpace)) {
                vtt->sp_trigger.receive_input_value(0);
            }
            break;

        case GDK_KEY_Tab:
            if (press_key(KEY_Tab)) {
                vtt_class::focus_next();
            }
            break;
        ...
        case GDK_KEY_s:
            if (press_key(KEY_s)) {
                vtt->sp_sync_follower.receive_input_value(!vtt->is_sync_follower);
            }
            break;
        ...
        case GDK_KEY_Control_L:
            if (press_key(KEY_Control_L)) {
                vtt->sp_mute.receive_input_value(1);
            }
            break;
        case GDK_KEY_Control_R:
            if (press_key(KEY_Control_R)) {
                vtt->sp_mute.receive_input_value(1);
            }
            break;

        case GDK_KEY_Alt_L:
            if (press_key(KEY_Alt_L)) {
                vtt->sp_mute.receive_input_value(0);
            }
            break;
        case GDK_KEY_Alt_R:
            if (press_key(KEY_Alt_R)) {
                vtt->sp_mute.receive_input_value(0);
            }
            break;
        ...
        case GDK_KEY_F2:
            if (press_key(KEY_F2)) {
                vtt_class::focus_no(1);
            }
            break;
        case GDK_KEY_F3:
            if (press_key(KEY_F3)) {
                vtt_class::focus_no(2);
            }
            break;
        case GDK_KEY_F4:
            if (press_key(KEY_F4)) {
                vtt_class::focus_no(3);
            }
            break;
        case GDK_KEY_F5:
            if (press_key(KEY_F5)) {
                vtt_class::focus_no(4);
            }
            break;
        case GDK_KEY_F6:
            if (press_key(KEY_F6)) {
                vtt_class::focus_no(5);
            }
            break;
        case GDK_KEY_F7:
            if (press_key(KEY_F7)) {
                vtt_class::focus_no(6);
            }
            break;
        case GDK_KEY_F8:
            if (press_key(KEY_F8)) {
                vtt_class::focus_no(7);
            }
            break;
        case GDK_KEY_F9:
            if (press_key(KEY_F9)) {
                vtt_class::focus_no(8);
            }
            break;
        case GDK_KEY_F10:
            if (press_key(KEY_F10)) {
                vtt_class::focus_no(9);
            }
            break;
        case GDK_KEY_F11:
            if (press_key(KEY_F11)) {
                vtt_class::focus_no(10);
            }
            break;
        case GDK_KEY_F12:
            if (press_key(KEY_F12)) {
                vtt_class::focus_no(11);
            }
            break;
        }
    }
...
void tx_mouse::key_release(GtkWidget* widget, GdkEventKey* eventKey) {
    if (vtt) {
        ...
        case GDK_KEY_m:
            if (release_key(KEY_m)) {
                vtt->sp_mute.receive_input_value(0);
            }
            break;
        case GDK_KEY_Control_L:
            if (release_key(KEY_Control_L)) {
                vtt->sp_mute.receive_input_value(0);
            }
            break;
        case GDK_KEY_Control_R:
            if (release_key(KEY_Control_R)) {
                vtt->sp_mute.receive_input_value(0);
            }
            break;

        case GDK_KEY_Alt_L:
            if (release_key(KEY_Alt_L)) {
                vtt->sp_mute.receive_input_value(1);
            }
            break;
        case GDK_KEY_Alt_R:
            if (release_key(KEY_Alt_R)) {
                vtt->sp_mute.receive_input_value(1);
            }
            break;

        case GDK_KEY_w:
            if (release_key(KEY_w)) {
                vtt->sp_mute.receive_input_value(0);
                warp = TX_MOUSE_SPEED_NORMAL;
                warp_override = false;
                vtt->set_scratch(0);
